...
| `nexa-coffee.ico` | Windows 리소스(16·32·48 · `res/`에 복사) — 256 PNG 프레임은 exe 크기 때문에 넣지 않았다 |
| `nexa-coffee.icns` | macOS 번들 |
...
magick /tmp/i16.png /tmp/i32.png /tmp/i48.png nexa-coffee.ico && cp nexa-coffee.ico ../../res/
mkdir -p /tmp/nc.iconset && for s in 16 32 128 256 512; do rsvg-convert -w $s -h $s icon.svg -o /tmp/nc.iconset/icon_${s}x${s}.png; rsvg-convert -w $((s*2)) -h $((s*2)) icon.svg -o /tmp/nc.iconset/icon_${s}x${s}@2x.png; done
