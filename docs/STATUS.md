...
- **T-14 ✅ 종료** — 메뉴 → 입력 창(**별도 자식 프로세스**) → Enter → **파이프 → 작업 시작**이 `powercfg` 억제 등록과 메뉴 남은 시간(`0일 0시간 44분 58초`, 설정 45분)으로 확인. DR-14 끝단 검증 완료.
- **T-2** — `powercfg /requests` DISPLAY·SYSTEM 등록 ✅ · 종료 시 해제 ✅ · 메뉴 라디오/체크 ✅ · **열린 메뉴 1초 갱신 ✅**(59:58 → 59:54 · 09-13 숙제). **남음**: 아이콘 DPI(배율 변경 필요) · TaskbarCreated(탐색기 실제 재시작 필요).
- **T-11** — Windows ✅(메뉴 CJK ko/ja/zh · 언어 메뉴 라디오 · 입력 창 라벨 폭 네 언어 전부 잘림 없음). **남음**: macOS 실기.
