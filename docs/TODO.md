...
| T-2 | P0 | 중 | Windows 실기 — 빌드·트레이·툴팁·ko 로케일 ✅ 09-13 · `powercfg /requests`(DISPLAY·SYSTEM + 해제) ✅ · 메뉴 라디오/체크 ✅ · 열린 메뉴 1초 갱신(59:58→59:54) ✅ 09-17 · **남음**: 아이콘 DPI(100/150/200% — 배율 변경 필요) · TaskbarCreated(탐색기 실제 재시작 필요) | ◐ |
...
| T-18 | P2 | 소 | **대기 중 DPI 변경 시 트레이 아이콘이 옛 크기로 남는다** — `job_tick()`은 매 틱 `icon_size()`를 다시 읽지만(동작 중 ✅), 대기 중엔 `show_idle()`을 부르는 계기가 없다. `WM_DPICHANGED`/`WM_SETTINGCHANGE`에서 `!running`이면 `show_idle(FALSE)` (09-17 코드 관찰) | ☐ |
