...
- **2026-09-17** — winget·choco 검수 점검: 둘 다 "대기"가 아니라 **우리 차례**였다. winget 설치 실패 원인을 Windows 실기로 규명 — Defender가 **x86 zip만** `Trojan:Win32/Tecabans.STV!cl`(클라우드 ML 오탐)로 격리(x64·매니페스트는 정상). 조치: PE 버전 리소스 추가(VERSION 단일 출처 · MSVC 30,720→31,744 B) · choco `iconUrl`→jsDelivr(모더레이터 Requirement) · 워크플로 `channels` 입력 · VERSION 0.1.1. → [journal](journal/2026-09-17.md)
