...
- 🔴 push 전: `make test` · `make win` · `sh scripts/linux-smoke.sh`(brew dbus) · macOS `make app` 실행 확인. 크기 예산(CI): Windows ≤ 64KB · Linux 정적 ≤ 128KB · macOS 유니버설 ≤ 256KB.
...
0. GitHub Actions 첫 실행 확인(`gh run watch`) — 크기 예산·MSVC·musl·D-Bus 스모크.
1. Windows 실기 확인(트레이 · DPI별 아이콘 크기 · 메뉴 라디오 표시 · 절전 억제 `powercfg /requests`).
2. Linux 실기 확인(KDE Plasma · GNOME AppIndicator) — `pixmap` 알파 해석, 서브메뉴, logind polkit.
